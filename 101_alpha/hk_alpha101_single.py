...
resample_period = '1D'  # pandas resample offset alias: the IC holding period ('1D' daily, '1W' weekly, '1ME' month-end, ...); override with --holding-period
rolling_period = 5  # default rolling window, in holding periods, for the IC chart; override with --window
...
min_periods_for_chart = 100  # an alpha needs at least this many valid IC periods to be eligible for the rolling-IC chart; override with --min-periods
...
def _windows(values, w):
    """(n, w) matrix of trailing w-windows, NaN-padded so row i ends at values[i]."""
...
def _rolling_np(x, w, kernel):
    """Apply a vectorized (n, w) -> (n,) kernel per column; avoids the Python-level
    per-window overhead of DataFrame.rolling().apply(), which doesn't scale past a
    handful of tickers/years."""
    out = {}
...
def load_data(csv_path, tickers=None, start=None, end=None, years=None):
    ...
    Ignored if --start is given explicitly.
    """
    path = Path(csv_path)
    if not path.exists():
    ...
    if start is None and years is not None and not df.empty:
...
# offset alias - default set by `resample_period` above): cross-sectional
...
def periodic_ic_series(factor, close, min_names=min_names, freq=resample_period):
...
def evaluate_alpha(name, factor, close, min_names=min_names, freq=resample_period):
...
def evaluate_all(factors: dict, close, min_names=min_names, freq=resample_period):
    rows = []
    ic_series = {}
...
def plot_rolling_ic(ic_series: dict, top_names, window=rolling_period, out_path="rolling_ic.png",
...
def main():
    p = argparse.ArgumentParser(description="Test WorldQuant 101 formulaic alphas on HK stocks (reads a local CSV, no network)")
    p.add_argument("--data", default="hk_universe_data.csv", help="CSV database built by download_hk_data.py")
    p.add_argument("--tickers", nargs="*", default=None, help="subset of tickers to test, e.g. 0700.HK 9988.HK")
    p.add_argument("--start", default=None, help="filter to dates on/after this (default: everything in the CSV)")
    p.add_argument("--end", default=None, help="filter to dates on/before this")
    p.add_argument("--years", type=float, default=None,
                    help="use only the most recent N years of the CSV's own date range (e.g. --years 5 out of "
                         "10 years cached) - a convenience alternative to computing --start by hand; ignored if --start is given")
    p.add_argument("--alphas", nargs="*", default=None, help="subset, e.g. alpha001 alpha012 alpha101 (default: all 101)")
    p.add_argument("--out", default="alpha_results_hk.csv")
    p.add_argument("--plot", default="rolling_ic_hk.png", help="output path for the rolling-IC chart")
    p.add_argument("--top-n", type=int, default=5, help="number of top alphas (by mean IC) to plot")
    p.add_argument("--holding-period", default=resample_period,
                    help="pandas resample offset alias setting the IC holding period, e.g. "
                         "'1D' (daily), '1W' (weekly), '1ME' (monthly) (default: %(default)s)")
    p.add_argument("--window", type=int, default=rolling_period,
    ...
    p.add_argument("--min-periods", type=int, default=min_periods_for_chart,
    ...
    open_df, high_df, low_df, close_df, volume_df, cap_df, sector, industry = load_data(
        args.data, args.tickers, args.start, args.end, years=args.years
    )
    ...
    plot_rolling_ic(ic_series, top_names, window=args.window, out_path=args.plot, freq=args.holding_period)
