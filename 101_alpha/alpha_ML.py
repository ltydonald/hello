...
STAGE              = "test"     # "tune" or "test"; override with --stage
...
LGB_PARAMS = dict(
    max_depth=3, learning_rate=0.005, n_estimators=500,
    min_samples_leaf=300, reg_lambda=5.0, random_state=42, verbose=-1,
)
...
# =============================================================================
# 3. Operators — Appendix A.2 of the paper
#
...
def scale(x, a=1.0):
    ...
    the damage isolated to the one ticker instead.
    """
...
def correlation(x, y, d):
    ...
    NOTE: a hand-rolled rolling-sums version (avoiding pandas' slower native
    `.rolling().corr()`) was tried here and reverted. The naive sum-of-squares
    formula (n*sum_x2 - sum_x**2) is a textbook-unstable way to compute variance:
    on real data, a window with a genuinely-constant price (undefined correlation)
    and a window with a real but tiny price tick (a well-defined correlation) can
    differ by as little as ~14x in the relative size of that subtraction - far too
    close to distinguish with any single epsilon threshold without either missing
    real degenerate cases or corrupting real small-signal ones. Fixing this
    properly needs a numerically-stable two-pass/Welford-style rolling algorithm,
    which is more work to vectorize correctly than was justified here; pandas'
    native implementation, despite being the slower path, already handles this
    correctly.
    """
...
def covariance(x, y, d):
    """Rolling sample covariance. Reverted to pandas' native implementation for
    the same numerical-stability reason as correlation() above - a hand-rolled
    version showed small (~1e-6) discrepancies against pandas on real data,
    consistent with the same class of floating-point instability."""
...
# =============================================================================
# 4. MarketData — panel container feeding the alpha formulas
# =============================================================================
...
# =============================================================================
# 5. The 101 formulaic alphas — Appendix A.1 of the paper
#
...
# (including redundant sub-expressions like `x*w + x*(1-w)`, kept for
# fidelity to the published formula text even though they simplify to `x`).
#
...
# Alphas that use indneutralize(x, IndClass.*) - need a sector/industry mapping.
...
def topk_ls(sub, pred_col, eval_col, k):
    """EW top-K long minus bottom-K short. Returns (gross_return, weights),
    where weights maps ticker -> signed portfolio weight (+1/k long, -1/k
    short) so the caller can measure turnover against the prior rebalance."""
    ...
    weights = pd.concat([
        pd.Series(1.0 / k, index=top["ticker"].values),
        pd.Series(-1.0 / k, index=bot["ticker"].values),
    ])
    return ret, weights


def walk_forward_ls(preds_df, pred_col, eval_col, k, cost_rate=TRANSACTION_COST):
    ...
    for m, grp in preds_df.groupby("date"):
        r, weights = topk_ls(grp, pred_col, eval_col, k)
        if np.isnan(r):
            continue
...
def main():
    ...
    parser.add_argument("--years", type=float, default=None,
                         help="use only the most recent N years of the cached data's own date range (e.g. "
                              ...
                              "already-cached CSV gets used for this run")
    ...
    if args.years is not None and not long_df.empty:
    ...
    preds_df = run_walk_forward(panel, x_cols, TRAIN_TARGET, EVAL_TARGET, eval_start,
                                 holding_days=HOLDING_DAYS, k=K, retrain_every=RETRAIN_EVERY)
    ls_ret = walk_forward_ls(preds_df, "pred", EVAL_TARGET, K, cost_rate=TRANSACTION_COST)
    ...
    stats = perf(ls_ret, periods_per_year, name=f"LightGBM 101-Alpha L/S ({eval_label}, net of cost)")
    print(f"\nLightGBM L/S ({eval_label}): {stats}")
    ...
    plot_strats({"LightGBM 101-Alpha L/S": ls_ret}, bench_ret, benchmark_ticker,
                periods_per_year, f"LightGBM L/S (K={K}, hold={HOLDING_DAYS}d) vs {benchmark_ticker}", out_path)
